...
## TODO: Audio Pool Architecture (planned)

Декаплінг запису від обробки — аудіо зберігається в тимчасові файли перед відправкою в модель.

### Концепція
- **Temp WAV pool** — останні 5 записів зберігаються як WAV в temp-директорії (живуть до reboot)
- Кожен запис спочатку зберігається у файл, потім передається в модель
- Аудіо ніколи не втрачається навіть при помилці моделі

### Обробка помилок
- **Помилка моделі** (CUDA OOM, crash, timeout) → авто-ретрай 1 раз
- **Повторна помилка** → ігнорувати, файл залишається в пулі
- **Галюцинація** → авто-ретрай НЕ допоможе (Whisper детермінований: той самий вхід = той самий вихід). Файл зберігається для ручного ретрая

### Трей меню
- Список 5 останніх записів (час, тривалість, статус: OK/error/hallucination)
- Клік по запису → повторна обробка через модель → результат в буфер обміну

### Майбутнє: fine-tuning на галюцинаціях
- Записи відмічені як галюцинація можна використати для LoRA fine-tuning
- Юзер вручну транскрибує правильний текст → пара (audio, correct_text) йде в training data
- Це єдиний спосіб "навчити" модель не галюцинувати на конкретних патернах голосу

### Технічні деталі
- Директорія: `%TEMP%\pushtotalk_pool\` або `tempfile.gettempdir()`
- Формат: WAV 16kHz mono float32
- Ротація: FIFO, max 5 файлів
- Розмір: ~5 хв × 16000 × 4 байти ≈ 19 МБ на файл, max ~95 МБ
